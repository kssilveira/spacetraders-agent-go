...
func all() error {
	...
	game := Game{Token: token, Client: &http.Client{}}
	headquarters, err := game.getHeadquarters()
	if err != nil {
		return err
	}
	contractTradeSymbols, err := game.acceptContract()
	if err != nil {
		return err
	}
	ship, err := game.buyShip(headquarters)
	if err != nil {
		return err
	}
	if err := game.navigate(headquarters, ship); err != nil {
		return err
	}
	if err := game.extract(ship, contractTradeSymbols); err != nil {
		return err
	}
	return nil
}
